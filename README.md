...
Through Next.js we can create an API endpoint as a Node.js serverless function it fits very well with a serverless database such as DynamoDB. The API Route in the application will be executed in a isolated environment in a scalable way, instead of the resource being served 24/7. This is handled by the cloud provider like AWS, it will up or down a isolated environment as demand.

Since serverless functions runs on server-side, [next-auth](https://next-auth.js.org/) is able to perform authentication stratagies, this project uses Github OAuth.
